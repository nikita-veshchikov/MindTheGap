...
\todo[inline]{The authors state that there is no extra cost for the RNG because the order is
not increased, how do you clear the registers? You just load unrelated values
from memory? It would have been interesting to also implement a 2nd order
version and compare the cost to just hardening. With just hardening you
do a lot of useless instructions while a smaller extra overhead might give you
a 2nd order implementation.  

I am a bit on the fence whether or not to accept this paper. There is nothing really wrong in the paper, but it is also not very exciting. I would have been interesting if more focus was put on the methodology to achieve the results, like a good methodology to learn all the ILA breaching effects, rather than listing the ILA breaching effects and then writing a tool to fix an implementation. The part I liked the most about the paper was the mentioning of the neighbouring leak effect.}
...
hence forcing the attacker to resort to 2nd-order and/or multivariate techniques\todo{Is this goal reached? Note that reaching this goal means being exhaustive on all the possible distance-based leakages that could occur which does not seem to be the case here (otherwise should be demonstrated).}. 
...
In order to ``harden" the Sbox, we use the solutions suggested in Section~\ref{sec:ila_effects} 
...
is 15 times slower\todo{What does explain the huge observed factor in the hardened implementations (between 12 and 15 times)?}. Table~\ref{cc_table} contains a comparison between ``naive" 1st-order, 
...
Nikita: we can point out the fact that e.g. we need less randomness}
...
\centering
 \begin{subfigure}[b]{0.47\textwidth}
        ...
        \caption{\scriptsize{Efficient hardened Sbox, 1st-order t-test, $25000$ random vs. $25000$ fixed.}}
\label{hardbox_eff}
    \end{subfigure}  \hspace{15px}
 \begin{subfigure}[b]{0.47\textwidth}
        \includegraphics[width=\textwidth]{{fig_pdf/hardbox_cons.pdf}}

        \caption{\scriptsize{Conservative hardened Sbox, 1st-order t-test, $10^{5}$ random vs. $10^{5}$ fixed.}}
\label{hardbox_cons}
    \end{subfigure}
...
\begin{subfigure}[b]{0.47\textwidth}
        ...
        \caption{\scriptsize{Consevative hardened Sbox, 2nd-order t-test, $25000$ random vs. $25000$ fixed.}}
\label{t_order2}
    \end{subfigure} \hspace{15px}
 \begin{subfigure}[b]{0.47\textwidth}
        \includegraphics[width=\textwidth]{{fig_pdf/softbox.pdf}}

        \caption{\scriptsize{Naive Sbox, 1st-order t-test, $1000$ random vs. $1000$ fixed.}}
\label{softbox}
    \end{subfigure}
