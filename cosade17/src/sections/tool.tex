...
\todo[inline]{While I do believe this tool could be of help with simple processors where
the leakage behavior and (micro-)architecture is easy to predict, I am a bit skeptic about the
usefulness of this all when it comes to larger processors (including
pipelining, out-of-order execution, etc.) 
Do you think it is feasible to get to a point where the tool would be able to
handle all leakages (even for advanced processors?). Do you think the
development of this tool could have added value for dealing with larger
processors?}

Several tools that can help designers of cryptographic systems
were already suggested and discussed in literature.\todo{add~\cite{DBLP:conf/ches/MossOPT12}}

\todo[inline]{The description of the tool ASCOLD is very high-level. The reader gets no clue on how the tool actually works. 
